          ...
          (if (= (length args) 2)
              (let ((item1 (drop (car args)))
                ...
                (if (= (type-tags item1) (type-tags item2))
                    ...
                    (let ((pos1 (tower-position item1))
                          (pos2 (tower-position item2)))
                      (if (> pos1 pos2)
                          (apply-generic op item1 (raise-to (type-tags item1) item2))
                          (apply-generic op (raise-to (type-tags item2) item1) item2))))))
          (error "No method for these types" (cons op type-tags))))))
